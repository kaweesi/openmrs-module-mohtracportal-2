...
<openmrs:require privilege="View TRAC Portal patients listing and export" otherwise="/login.htm" redirect="/module/mohtracportal/patientList.htm?page=1" />

<h2><spring:message code="mohtracportal.patientlist.title"/></h2><br/>
	<div>	
		<div style="float: left; width: 60%;">
			<form action="patientList.htm" method="get">
				<table>
					<tr>
						<td><spring:message code="mohtracportal.from"/></td>
						<td><img border="0" src="<openmrs:contextPath/>/images/info.gif" title="<spring:message code="mohtracportal.help.from"/>"/></td>
						<td><input value="${param.from}" size="11" type="text" onclick="showCalendar(this)" name="from"/></td>
						<td style="margin-left: 10px;"><spring:message code="mohtracportal.to"/></td>
						<td><img border="0" src="<openmrs:contextPath/>/images/info.gif" title="<spring:message code="mohtracportal.help.to"/>"/></td>
						<td><input value="${param.to}" size="11" type="text" onclick="showCalendar(this)" name="to"/></td>
						<td><input type="submit" class="list_exportBt" value="<spring:message code="mohtracportal.refresh"/>" title="<spring:message code="mohtracportal.refresh"/>" id="btRefresh"/></td>
	...
	<div id="usersId" style="margin-top: 4px; text-align: left; padding-left: 7px;" class="menuTab" title="<spring:message code="mohtracportal.show"/>"><spring:message code="mohtracportal.selected"/> : <span id="nbrOfUsers"><spring:message code="mohtracportal.all"/></span> <spring:message code="mohtracportal.users"/></div>
	<div id="usersList" style="margin-bottom: 5px;"><br/>
		<input type="hidden" value="<spring:message code='mohtracportal.checkAll'/>" id="checkAllWord"/>
		<input type="button" checked="checked" value="<spring:message code="mohtracportal.uncheckAll"/>" onclick="checking()" name="chkbxUsr_all" id="chkbxUsrId_all"/>
		<table>
			<tr>
				<td>
					<div class="listUser">
		...
		<div id="close" class="menuTab" style="text-align: left; padding-left: 7px;" title="<spring:message code="mohtracportal.hide"/>"><spring:message code="mohtracportal.hide"/></div>
	</div>
	...
	<div style="margin: auto; width: 80%; font-size: 0.9em;">
		<div id="list_container" style="width: 99%">
			<div id="list_title">
				<div class="list_title_msg"><spring:message code="mohtracportal.patient.listing"/></div>
					...
					<span>
						<openmrs:hasPrivilege privilege="Export Collective Patient Data">
							<form action="patientList.htm?page=1&export=csv${parameters}" method="post" style="display: inline;">
								<input type="submit" class="list_exportBt" value="<spring:message code="mohtracportal.patientlist.xprt.csv"/>" title="<spring:message code="mohtracportal.patientlist.csv"/>"/>
							</form>&nbsp;&nbsp;
							<form action="patientList.htm?page=1&export=pdf${parameters}" method="post" style="display: inline;">
								<input type="submit" class="list_exportBt" value="<spring:message code="mohtracportal.patientlist.xprt.pdf"/>" title="<spring:message code="mohtracportal.patientlist.pdf"/>"/>
							</form>
			...
			<table id="list_data">
				<tr>
					...
					<openmrs:hasPrivilege privilege="View Patient Names">
						<th width="30%" class="columnHeader"><spring:message code="mohtracportal.patient.names"/></th>
					</openmrs:hasPrivilege>
					...
					<th width="20%" class="columnHeader"><spring:message code="mohtracportal.numberOfEncounters"/></th>
				</tr>
				<c:if test="${empty patientList}"><tr><td colspan="6" width="100%" style="text-align: center;"><spring:message code="mohtracportal.tablelist.empty"/></td></tr></c:if>
				...
				<c:forEach items="${patientList}" var="patient" varStatus="status">
					<tr>
						...
						<td class="rowValue ${status.count%2!=0?'even':''}">${mohtractag:numberOfEncounterByPatient(patient[0])}&nbsp;<spring:message code="mohtracportal.encounters"/></td>
					</tr>
				</c:forEach>
			</table>
			<div id="list_footer">
						...
						<tr>
							<c:if test="${prevPage!=-1}">
								<td width="100px" class="" style="padding:1px 2px 1px 2px; vertical-align: text-top;">
									<a href="patientList.htm?page=1${parameters}"><div class="list_pageNumber" style="text-align: center;">|&lt; <spring:message code="mohtracportal.first"/></div></a>
								</td>
							...
							<c:if test="${nextPage!=-1}">
								...
								<td width="100px" class="" style="padding:1px 2px 1px 2px; vertical-align: text-top;">
									<a href="patientList.htm?page=${lastPage}${parameters}"><div class="list_pageNumber" style="text-align: center;"><spring:message code="mohtracportal.last"/> &gt;|</div></a>
								</td>
							</c:if>
						</tr>
					</table>		
	...
	<script>
		...
		function checking(){
			...
			if(document.getElementById("chkbxUsrId_all").value==$("#checkAllWord").val()){
				...
				document.getElementById("chkbxUsrId_all").value="<spring:message code='mohtracportal.uncheckAll'/>";
			}else {
				while(document.getElementById("chkbxUsrId_"+i)!=null){
					document.getElementById("chkbxUsrId_"+i).checked=false;
					i++;
				}
				document.getElementById("chkbxUsrId_all").value="<spring:message code='mohtracportal.checkAll'/>";
			}
			checkStatus();
		}
		...
		function checkStatus(){
			...
			$("#nbrOfUsers").html(((notChecked)?checked:"<spring:message code='mohtracportal.all'/>"));
